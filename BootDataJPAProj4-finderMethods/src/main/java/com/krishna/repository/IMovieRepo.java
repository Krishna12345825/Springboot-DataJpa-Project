...
public interface IMovieRepo extends JpaRepository<Movie, Integer> {
	...
	// select mid,mname,year,ratings from Movie where mname like 'R%' // movies
	// starting with R
	// select mid,mname,year,ratings from Movie where mname like '___' // 3 letter
	// movies
	// select mid,mname,year,ratings from Movie where mname like '%R%' //Containing
	// letter R
	// select mid,mname,year,ratings from Movie where mname like '%R' //ending
	// letter R
	...
	// select mid,mname,year,ratings from Movie where Mname like 'R%' or (ratings
	// between 3.0 and 5.0)
	public Iterable<Movie> findByMnameStartingWithOrRatingBetween(String mnameChars, float startRating,
			float endRatings);

	// select mid,mname,year,ratings from Movie where year in (? , ?) or
	// (upper(mname) like upper(?) escape ?) and (ratings between ? and ?)
	public Iterable<Movie> findByYearInOrMnameContainingIgnoreCaseAndRatingBetween(List<String> years, String chars,
			float startRatings, float endRatings);

	// select mid,mname,year,ratings from Movie where (mname not like ? escape ?)
	// and (year in (? , ?)) order by mname asc
